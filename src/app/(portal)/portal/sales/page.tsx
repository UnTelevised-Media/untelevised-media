...
import OrdersTable, { type OrderWithItems } from '@/components/portal/OrdersTable';
...
export default async function PortalSalesPage() {
  ...
  try {
    ...
    const earningsQueryBuilder =
      role === 'author'
        ? shopServiceClient
            .from('author_earnings')
            .select(
              'order_id, gross_cents, stripe_fee_cents, net_after_stripe_cents, author_cents, platform_cents, publisher_cents, is_tip'
            )
    ...
    orders = allOrders
      ...
      .map((o) => ({
        ...
        items: o.items.map((i) => ({
          ...
          sanity_format_type:
            i.sanity_format_type as OrderWithItems['items'][0]['sanity_format_type'],
          quantity: i.quantity,
            ...
            {(() => {
              ...
              const tipItemCents = (o: OrderWithItems) =>
                o.items
                  .filter((i) => i.sanity_format_type === 'tip')
                  .reduce((s, i) => s + i.unit_price_cents * i.quantity, 0);
              const tipOnlyOrders = active.filter(
                (o) => o.items.length > 0 && o.items.every((i) => i.sanity_format_type === 'tip')
              );
              const bookOrders = active.filter((o) =>
              ...
              const avgCents =
                bookOrders.length > 0 ? Math.round(grossBookCents / bookOrders.length) : 0;
              const now = new Date();
              const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
              const lastMonthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1);
              const thisMonthBookCents = bookOrders
                .filter((o) => new Date(o.created_at) >= monthStart)
                .reduce((s, o) => s + bookItemCents(o), 0);
              const lastMonthBookCents = bookOrders
                .filter((o) => {
                ...
                .reduce((s, o) => s + bookItemCents(o), 0);
              const monthDelta =
                lastMonthBookCents > 0
                  ? Math.round(
                      ((thisMonthBookCents - lastMonthBookCents) / lastMonthBookCents) * 100
                    )
                  : null;
              ...
              const shippedDelivered = orders.filter((o) =>
                ['shipped', 'delivered', 'fulfilled'].includes(o.status)
              ).length;
              const digitalSold = active.reduce(
                (s, o) =>
                  s + o.items.filter((i) => i.is_digital).reduce((ss, i) => ss + i.quantity, 0),
                0
              );
                ...
                <>
                  <div className='mb-3 grid grid-cols-2 gap-3 sm:grid-cols-4'>
                    <MiniStat
                      ...
                      sub={
                        tipOnlyOrders.length > 0
                          ? `${bookOrders.length} book · ${tipOnlyOrders.length} tip-only`
                          : `${orders.length} total`
                      }
                    />
                    <MiniStat
                      label='Book Revenue'
                      value={fmt(grossBookCents)}
                      sub='excl. tips & cancelled'
                      accent='green'
                    />
                    <MiniStat
                      label='This Month'
                      value={fmt(thisMonthBookCents)}
                      sub={
                        monthDelta !== null
                          ? `${monthDelta >= 0 ? '+' : ''}${monthDelta}% vs last month`
                          : 'first month of data'
                      }
                      accent={monthDelta !== null && monthDelta >= 0 ? 'green' : 'red'}
                    />
                    <MiniStat
                      label='Avg Book Order'
                      value={fmt(avgCents)}
                      sub={`across ${bookOrders.length} order${bookOrders.length !== 1 ? 's' : ''}`}
                    />
                  </div>
                  <div className='mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4'>
                    <MiniStat
                      label='Needs Shipping'
                      value={String(needsShipping)}
                      sub={`${shippedDelivered} shipped / delivered`}
                      accent={needsShipping > 0 ? 'amber' : undefined}
                    />
                    <MiniStat
                      label='Digital Sold'
                      value={String(digitalSold)}
                      sub='items (all time)'
                    />
                    <MiniStat
                      label='Tips'
                      value={fmt(grossTipsCents)}
                      sub={
                        tipOnlyOrders.length > 0
                          ? `${tipOnlyOrders.length} standalone order${tipOnlyOrders.length !== 1 ? 's' : ''}`
                          : 'from book orders'
                      }
                      accent='green'
                    />
                    <MiniStat
                      ...
                      sub={refundCount > 0 ? 'review required' : 'none issued'}
                      accent={refundCount > 0 ? 'red' : undefined}
                    />
                  </div>
                </>
              );
            ...
            {(isAdmin || isSales) && earningsBreakdown.gross > 0 && (
              <section className='mb-6'>
                ...
                <div className='grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6'>
                  <MiniStat
                    label='Gross Revenue'
                    value={fmt(earningsBreakdown.gross)}
                    sub='What customers paid'
                  />
                  <MiniStat
                    label='Stripe Fees'
                    value={`− ${fmt(earningsBreakdown.stripeFees)}`}
                    sub='Processing (all cards)'
                    accent='red'
                  />
                  <MiniStat
                    label='Net to Platform'
                    value={fmt(earningsBreakdown.netAfterStripe)}
                    sub='After Stripe deduction'
                    accent='green'
                  />
                  <MiniStat
                    label='Author Payouts'
                    value={fmt(earningsBreakdown.authorCents)}
                    sub='All authors combined'
                  />
                  {isAdmin && (
                    <MiniStat
                      label='Platform Revenue'
                      value={fmt(earningsBreakdown.platformCents)}
                      sub='Our cut (after Stripe)'
                      accent='green'
                    />
                  )}
                  {isAdmin && (
                    <MiniStat
                      ...
                      value={fmt(earningsBreakdown.publisherCents)}
                      sub='Publisher cut (after Stripe)'
                    />
                  )}
                </div>
              </section>
