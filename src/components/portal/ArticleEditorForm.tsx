...
export default function ArticleEditorForm({
  ...
  // Selected sources
  const [selectedSources, setSelectedSources] = useState<SourceRef[]>(initialData?.sources ?? []);
  ...
  const [faqs, setFaqs] = useState<{ _key: string; question: string; answer: string }[]>(() =>
    (
      (initialData?.faqs as { _key?: string; question: string; answer: string }[] | undefined) ??
      []
    ).map((f) => ({ ...f, _key: f._key ?? Math.random().toString(36).slice(2, 10) }))
  );
  ...
  const initCorrection = (initialData as Record<string, unknown>)?.correction as
    | CorrectionState
    | undefined;
  ...
  const [correction, setCorrection] = useState<CorrectionState>(
    initCorrection
      ? { ...initCorrection, issuedAt: initCorrection.issuedAt?.slice(0, 16) ?? '' }
      : null
  );
      ...
      <form className='space-y-8' onSubmit={(e) => e.preventDefault()}>
            ...
            {mainImage && (
              <div>
                ...
                <Input
                  ...
                  value={mainImage.alt}
                  onChange={(e) =>
                    setMainImage((prev) => (prev ? { ...prev, alt: e.target.value } : prev))
                  }
        ...
        <div className='grid gap-6 sm:grid-cols-2'>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSelectedCategories((prev) =>
                        selected ? prev.filter((c) => c._id !== cat._id) : [...prev, cat]
                      )
                    }
          ...
          <section>
            ...
            <Input id='location' {...register('location')} placeholder='e.g. Atlanta, GA' />
          </section>
                  ...
                  {articleResults.map((a) => {
                    ...
                    return (
                      <li key={a._id}>
                        ...
                        >
                          {a.title}
                          {alreadyLinked && <span className='ml-2 text-xs text-green-600'>✓</span>}
                        </button>
                      </li>
                    );
                  })}
          ...
          <div>
            ...
            <div className='space-y-3'>
              {faqs.map((faq, i) => (
                <div
                  key={i}
                  className='space-y-2 border border-slate-200 p-3 dark:border-slate-700'
                >
                  <div className='flex items-center gap-2'>
              ...
              <Button
                ...
                onClick={() =>
                  setFaqs((prev) => [
                    ...prev,
                    { _key: Math.random().toString(36).slice(2, 10), question: '', answer: '' },
                  ])
                }
              >
                + Add FAQ
              ...
              <div className='mt-3 space-y-4 border border-slate-200 p-4 dark:border-slate-700'>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value='clarification'>
                        Clarification — added context, no error
                      </SelectItem>
                      <SelectItem value='update'>Update — new developments added</SelectItem>
                      <SelectItem value='retraction'>
                        Retraction — article fully withdrawn
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCorrection((prev) => (prev ? { ...prev, summary: e.target.value } : null))
                    }
                    maxLength={120}
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setCorrection((prev) => (prev ? { ...prev, detail: e.target.value } : null))
                    }
                    rows={4}
