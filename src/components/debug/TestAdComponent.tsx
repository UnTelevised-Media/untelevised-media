...
export default function TestAdComponent({
  ...
  if (!isClient) {
    return <div className='rounded border p-4'>Loading test ad component...</div>;
  }
  ...
  return (
    <div className={`rounded-lg border-2 p-4 ${getStatusColor()}`}>
      ...
      <div className='mb-4 border border-dashed border-gray-300 p-2'>
        <div className='mb-1 text-xs text-gray-500'>Ad Container:</div>
        <ins
          ref={adRef}
      ...
      <div className='space-y-2'>
        ...
        <div className='text-sm'>
          <strong>Consent:</strong> {hasConsent ? '✅' : '❌'} |<strong> Marketing:</strong>{' '}
          {canUseMarketing ? '✅' : '❌'}
        </div>

        <details className='text-xs'>
          <summary className='cursor-pointer font-medium'>View Logs ({logs.length})</summary>
          <div className='mt-2 max-h-32 overflow-y-auto rounded bg-gray-100 p-2 dark:bg-gray-800'>
            {logs.map((log, index) => (
