...
const Nav = async () => {
  ...
  const secondaryCategories = sortedCategories.slice(4);

  return (
    <nav className='from-slate-900/98 to-slate-800/98 sticky top-[73px] z-40 border-b border-slate-700/50 bg-gradient-to-r shadow-xl backdrop-blur-md'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        <div className='flex items-center justify-between py-4'>
          ...
          <div className='flex items-center space-x-4'>
            ...
            <div className='hidden items-center space-x-1 md:flex'>
              ...
              <span className='text-xs text-slate-400'>Live Coverage Active</span>
            </div>
          </div>
            ...
            {primaryCategories.map((category: any, index: number) => (
              <React.Fragment key={category._id}>
                <ClientSideRoute route={`/category/${formatCategoryTitle(category.title)}`}>
                  <div className='group relative flex cursor-pointer items-center space-x-2 whitespace-nowrap rounded-lg border border-transparent px-6 py-2.5 text-sm font-medium text-slate-300 transition-all duration-300 hover:border-slate-600/50 hover:bg-slate-700/30 hover:text-white hover:shadow-lg'>
          ...
          {secondaryCategories.length > 0 && (
            <div className='group relative hidden lg:block'>
              <button className='flex items-center space-x-2 rounded-lg border border-slate-600/50 bg-slate-800/30 px-4 py-2 text-sm font-medium text-slate-300 backdrop-blur-sm transition-all duration-200 hover:border-untele/50 hover:bg-slate-700/50 hover:text-white'>
                <span>More</span>
                <ChevronRight className='h-4 w-4 transition-transform duration-200 group-hover:rotate-90' />
              </button>

              {/* Dropdown Menu */}
              <div className='absolute right-0 top-full mt-2 w-64 rounded-lg border border-slate-600/50 bg-slate-800/95 p-2 opacity-0 shadow-2xl backdrop-blur-md transition-all duration-200 group-hover:opacity-100'>
                <div className='grid grid-cols-2 gap-2'>
                  {secondaryCategories.map((category: any) => (
                    ...
                    >
                      <div className='group/item cursor-pointer rounded-md px-3 py-2 text-sm text-slate-300 transition-colors duration-200 hover:bg-slate-700/50 hover:text-white'>
        ...
        <div className='border-t border-slate-700/30 py-3 lg:hidden'>
          <div className='flex space-x-2 overflow-x-auto scrollbar-hide'>
            {secondaryCategories.map((category: any) => (
              <ClientSideRoute
                ...
                route={`/category/${formatCategoryTitle(category.title)}`}
              >
                <div className='flex cursor-pointer items-center space-x-1 whitespace-nowrap rounded-full border border-slate-600/50 bg-slate-800/30 px-3 py-1 text-xs text-slate-400 backdrop-blur-sm transition-all duration-200 hover:border-untele/50 hover:bg-untele/10 hover:text-white'>
        ...
        {/* Activity Indicator */}
        <div className='border-t border-slate-700/20 py-2'>
