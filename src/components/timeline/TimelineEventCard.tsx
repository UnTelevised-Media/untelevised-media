...
const TimelineEventCard: React.FC<TimelineEventCardProps> = ({
    ...
    return (
      <div className={`timeline-event-card-compact ${className}`}>
        <Link href={`/timeline/event/${event.slug.current}`}>
          <div className="group flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 hover:border-untele/50 transition-all dark:border-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700">
            {/* Event Image */}
            {event.mainImage && (
            ...
            <div className="flex-1 min-w-0">
              ...
              <h4 className="font-medium text-sm line-clamp-1 group-hover:text-untele transition-colors">
                {event.title}
              </h4>
  ...
  return (
    <div className={`timeline-event-card ${className}`}>
      <div className={`
        group relative overflow-hidden rounded-lg border-2 bg-white shadow-sm transition-all duration-300 
        hover:shadow-lg hover:scale-[1.02] dark:bg-slate-800
        ${getImportanceStyle(event.importanceLevel)}
          ...
          {showRelated && (event.relatedArticles?.length || event.relatedLiveEvents?.length) && (
            <div className="text-xs text-slate-600 dark:text-slate-400">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {event.relatedArticles?.length || 0} article{(event.relatedArticles?.length || 0) !== 1 ? 's' : ''}
                  {event.relatedLiveEvents?.length && (
                    <>, {event.relatedLiveEvents.length} live event{event.relatedLiveEvents.length !== 1 ? 's' : ''}</>
                  )}
                </span>
              </div>
            </div>
