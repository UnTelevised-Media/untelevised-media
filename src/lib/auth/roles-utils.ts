...
// Safe to import in any context (server, client, tests).
//
// Role hierarchy: admin > editor > author > sales
// 'sales' is a restricted role: can only access /portal/orders, nothing else.
...
export function getRoleFromMeta(meta: Record<string, unknown>): PortalRole | null {
  ...
  // Backwards-compat: legacy `admin: true` flag
...
export function hasRole(role: PortalRole | null, required: PortalRole): boolean {
  ...
  // required === 'sales' — sales satisfies only 'sales' checks; not author, editor, admin
  if (required === 'sales') return role === 'sales' || role === 'admin';
  return false;
}

/** Returns true if the role has any portal access (any non-null role). */
