...
export async function uploadBookCover(formData: FormData): Promise<string> {
  ...
  if (!bookId || !file) {
    throw new Error('Missing bookId or file');
  }
...
export async function uploadDigitalAsset(formData: FormData): Promise<string> {
  ...
  const file = formData.get('file') as File | null;
  if (!bookId || !formatKey || !file) {
    throw new Error('Missing bookId, formatKey, or file');
  }
...
export async function updateBook(bookId: string, input: UpdateBookInput): Promise<void> {
  ...
  if (input.isbn?.trim()) {
    patch.isbn = input.isbn.trim();
  } else if (input.isbn !== undefined) {
    unset.push('isbn');
  }
  ...
  if (input.fictionType !== undefined) {
    if (input.fictionType) {
      patch.fictionType = input.fictionType;
    } else {
      unset.push('fictionType');
    }
  }
  ...
  if (input.formatPrices?.length) {
    for (const {
      ...
      if (stripePriceId !== undefined) {
        if (stripePriceId) {
          patch[`formats[_key=="${key}"].stripePriceId`] = stripePriceId;
        } else {
          unset.push(`formats[_key=="${key}"].stripePriceId`);
        }
      }
      if (stripeProductId !== undefined) {
        if (stripeProductId) {
          patch[`formats[_key=="${key}"].stripeProductId`] = stripeProductId;
        } else {
          unset.push(`formats[_key=="${key}"].stripeProductId`);
        }
      }
    }
  }
...
export async function createBookGenre(title: string, slug: string): Promise<SanityBookGenre> {
  ...
  if (!title.trim()) {
    throw new Error('Genre title is required.');
  }
  if (!slug.trim()) {
