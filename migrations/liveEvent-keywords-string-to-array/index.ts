/**
 * Migration: liveEvent keywords string → array
 *
 * Converts the `keywords` field on all `liveEvent` documents from a plain
 ...
 *
 * Run with:
 *   pnpm sanity migration run liveEvent-keywords-string-to-array
 *
 * Dry-run (no writes):
 *   pnpm sanity migration run liveEvent-keywords-string-to-array --dry-run
 */
import { at, defineMigration, patch, set } from 'sanity/migrate';

export default defineMigration({
  title: 'liveEvent keywords: string → array',
  documentTypes: ['liveEvent'],
